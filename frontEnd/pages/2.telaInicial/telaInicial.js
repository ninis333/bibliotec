...
const campoPesquisa = document.getElementById("inputPesquisarLivros");
...
function montarCategoria(titulo, genero, dados, favoritos) {
  ...
  a.href = "../11.telaCategoria/telaCategoria.html";

  h1.id = `h1${titulo}`;
  h1.textContent = titulo;
  a.addEventListener("click", () => {
    localStorage.setItem("categoriaSelecionada", genero);
  });
  a.appendChild(h1);
  ...
  dados.forEach((livro) => {
    ...
    card.dataset.genero = livro.genero;
    ...
    card.innerHTML = `
            <img src="${livro.capa_url}" alt="${
      livro.titulo
    }" class="livro" data-descricao="${livro.descricao}" data-genero="${
      livro.genero
    }" id="livro-${livro.id}">
...
campoPesquisa.addEventListener("input", () => {
  const valor = campoPesquisa.value.toLowerCase();

  document.querySelectorAll(".fade-up").forEach((fade) => {
    const cards = fade.querySelectorAll(".cardLivro");

    let algumVisivel = false;

    cards.forEach((card) => {
      const { titulo, genero, autor } = card.dataset;
      const texto = `${titulo} ${genero} ${autor}`.toLowerCase();

      const corresponde = texto.includes(valor);
...
// filtro de genero
const selectGenero = document.querySelectorAll('input[type="checkbox"]');

const botaoFiltrar = document.getElementById("botaoFiltrar");

botaoFiltrar.addEventListener("click", () => {
    const generosSelecionados = Array.from(selectGenero)
    ...
    let algumVisivel = false;
    cards.forEach((card) => {
      const { genero } = card.dataset;
      const corresponde = generosSelecionados.length === 0 || generosSelecionados.includes(genero);
        card.style.display = corresponde ? "" : "none";
        if (corresponde) algumVisivel = true;
    });
